...
- `if (!user || !user.passwordHash)` — no account at all, or a Google-only
  account with no password to check. This branch now runs
  `comparePassword(input.password, await dummyPasswordHash)` *before*
  throwing, discarding the result entirely. This closes a genuine timing
  side channel that existed here previously: `comparePassword` (bcrypt) is
  deliberately slow (§5) — tens of milliseconds — so a real "wrong password"
  attempt (further down, where `comparePassword` runs against `user.passwordHash`)
  always took measurably longer to respond than this branch used to, which
  returned its `401` immediately with no bcrypt call at all. That timing gap
  is a textbook account-enumeration primitive: even though the response
  *body* is identical either way, an attacker can still distinguish
  "registered email, wrong password" from "email not registered" purely by
  how long the response took — no different from the classic "invalid
  username" vs. "invalid password" message split, just moved into the
  timing channel instead of the response text. `dummyPasswordHash` is a
  module-level `hashPassword(...)` call — computed once, lazily, the first
  time this module is loaded — rather than a hardcoded bcrypt-hash literal,
  specifically so it goes through the exact same cost factor as every real
  hash in the database (`SALT_ROUNDS`, §5) and can never silently drift out
  of sync with it if that constant is ever retuned.
...
- **A residual, accepted timing gap:** the `isActive`/`isLocked` checks above
  still return their `401` without ever calling `comparePassword`, so a
  disabled or currently-locked account is still measurably faster to reject
  than a "wrong password against a normal account" attempt — unlike the
  "no such account" case just fixed above. This is a smaller, deliberate
  trade rather than an oversight: reaching either of these branches already
  requires the email to belong to a real, existing account (the "does this
  email exist at all" question — the one meaningfully useful signal to an
  outside attacker — is what the dummy-hash comparison above closes), and
  skipping bcrypt here doubles as a real defense: it means a locked-out
  account can't be hammered with guesses *during* its lockout window at
  bcrypt's cost, and a disabled account never pays for a comparison that can
  never succeed regardless of the result.
...
- `nonce` — also generated by the controller and stashed alongside `state`.
  Google embeds whatever value is passed here as the `nonce` *claim* inside
  the signed ID token it later returns. `exchangeCodeForProfile` (below)
  checks that claim against the value the controller expects back, closing a
  gap `state` alone doesn't: `state` only proves *this specific browser
  round-trip* is legitimate, not that the ID token handed back at the end of
  it is the one minted *for this specific auth request* rather than some
  other still-valid Google-issued token.
- `code_challenge`/`code_challenge_method: 'S256'` — PKCE (RFC 7636). The
  controller generates a random `code_verifier`, hashes it (SHA-256,
  base64url) into this `code_challenge`, and sends only the challenge here.
  Google remembers it and will refuse to hand out tokens for this
  authorization at all unless the token-exchange call in
  `exchangeCodeForProfile` presents the matching original `code_verifier` —
  so possessing the one-time `code` alone is no longer enough to redeem it.
...
1. `client.getToken({ code, codeVerifier })` — the backend, authenticating
   itself with `GOOGLE_CLIENT_SECRET` *and* proving it holds the PKCE
   `codeVerifier` matching the `code_challenge` sent at the start of the
   flow, trades the one-time `code` for Google's own tokens, including an
   `id_token` (a JWT *Google* signed, describing the logged-in user).
2. `client.verifyIdToken({ idToken, audience: ... })` — cryptographically
   ...
   replayed here).
- `payload.nonce !== expectedNonce` — checked *after* signature/audience
  verification, so this comparison only ever runs against a claim we know
  Google actually signed. `verifyIdToken` has no idea what nonce *this app*
  expects for *this specific* login attempt — that context only exists here,
  in the caller-supplied `expectedNonce` threaded down from the cookie the
  controller set before redirecting to Google.
...
1. Generates three single-use values for this one login attempt: a random,
   unguessable `state` (CSRF), a random `nonce` (ID token replay defense),
   and a PKCE `codeVerifier`/`codeChallenge` pair (RFC 7636 — authorization
   code interception defense). `generatePkcePair()` hashes 32 random bytes
   (base64url, 43 characters — within RFC 7636's required 43-128 range) with
   SHA-256 to derive the challenge; only the *challenge* goes to Google here,
   the *verifier* stays server-side (in the cookie) until the token exchange.
2. Bundles all three into one JSON-encoded value and stores it in a single
   cookie on the user's browser — `httpOnly` (JavaScript on the page can't
   read it, blocking XSS from stealing it), `secure` in production (only
   sent over HTTPS), `sameSite: 'lax'` (still sent on the top-level redirect
   *back* from Google, which is what's needed here, while still blocking
   most cross-site abuse), and `signed: true` (Express signs it with
   `COOKIE_SECRET` so it can detect if the cookie value was tampered with
   client-side). One cookie rather than three separate ones — simpler to
   keep in sync, and all three values share exactly the same lifetime and
   trust requirements anyway, so there's nothing to gain from splitting them.
3. Redirects the browser to Google, embedding `state`, `nonce`, and
   `code_challenge`/`code_challenge_method=S256` in the URL — see
   `google.service.ts` (§11) for what each one does once Google's redirect
   comes back.
...
function parseOAuthCookie(raw: unknown): OAuthCookiePayload | undefined {
  ...
  try {
    ...
    if (
      ...
      typeof (parsed as any).state === 'string' &&
      typeof (parsed as any).nonce === 'string' &&
      typeof (parsed as any).codeVerifier === 'string'
    ) {
      return parsed as OAuthCookiePayload;
    }
  } catch { /* malformed JSON — treated as missing below */ }
  return undefined;
}
...
```
`parseOAuthCookie` turns the raw cookie value back into `{ state, nonce,
codeVerifier }`, but treats *any* problem — cookie absent, signature
verification failed (cookie-parser sets the signed-cookie entry to `false`
in that case, which fails the `typeof raw !== 'string'` check), or the JSON
doesn't parse or doesn't have the three expected string fields — identically:
`undefined`, handled by the same "invalid or missing OAuth state" branch
below as a plain missing cookie always was. There's no new failure mode to
special-case just because the cookie now carries structured data instead of
one bare string.
...
request that ordering never actually broke anything — `cookiePayload` is read
...
victim's cookies, which they don't. `state` is what stops this specific
login-CSRF attack; it's `nonce` and PKCE's `codeVerifier` (also pulled out of
this same cookie payload here, and passed on to `loginWithGoogleCode` next)
that handle the two other things a bare `state` check doesn't cover — see
`google.service.ts` (§11) for what each of those actually defends against.

Once state is verified, `loginWithGoogleCode(code, codeVerifier, nonce)` does
the real work (§11) and returns the real `user`/`tokens` — but notice those never reach
...
- `cookieParser(env.COOKIE_SECRET)` — parses the `Cookie` header into
  ...
  the OAuth cookie (which carries `state`, `nonce`, and the PKCE
  `codeVerifier` together as one signed JSON value — see §12).
